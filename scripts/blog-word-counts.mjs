...
// Generated, never hand-edited: blog id -> word count, for the "5 min read"
// label on /blogs. The ledger JSON already holds this, but it is 128 KB and a
// list page should not fetch a report to print one number; ~107 pairs bundle
// into the app for about 1.5 KB.
const WORDS_FILE = path.join(ROOT, "src", "data", "blogWords.js");
...
async function main() {
  ...
  const wordsById = Object.fromEntries(
    report.posts
      .filter((p) => p.blogId && p.words)
      .map((p) => [p.blogId, p.words]),
  );
