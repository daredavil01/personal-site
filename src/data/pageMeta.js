...
// Every `image` below is the COMMITTED fallback card in public/og/. The real
// card is rendered on demand at /api/og/... — consumers upgrade these URLs via
// `ogCardUrl` in src/lib/og/paths.js. Keeping the static URL here means
// OG_MODE=static works by simply not upgrading, and that a route always has a
// real 1200x630 image even if rendering is off or fails.
export const DEFAULT_IMAGE = `${SITE_URL}/og/home.png`;
export const NOT_FOUND_IMAGE = `${SITE_URL}/og/notfound.png`;

// Retained because the portrait is still the source image composited into the
// identity cards (/, /about, /resume) by src/lib/og/model.js.
...
// Each caller extracts the item fields (client camelCase / REST snake_case),
// resolves the card URL with `ogCardUrl` from src/lib/og/paths.js, and passes
// normalised primitives in.
//
...
//   - `image` falls back to the section's committed card, never to a bare logo,
//     so a failed lookup still unfurls as the right section
...
// site's name.
//
// This is the one card that ONLY on-demand rendering makes possible: a share is
// created by a reader at request time and pasted into a chat seconds later, so
// nothing generated ahead of time could ever contain it.
