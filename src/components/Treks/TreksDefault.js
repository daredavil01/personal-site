...
const difficultyOverlayClass = (level) => {
  switch (level?.toLowerCase()) {
    ...
    default:
      return "bg-stone-700/80 text-stone-200";
  }
};
  ...
  const filtered = useMemo(() => {
    ...
    if (filterYear !== "all") {
      result = result.filter(
        (t) => parseDate(t.date).getFullYear().toString() === filterYear,
      );
    }
    if (filterBlog === "with") {
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6">
          {filtered.map((trek) => (
            ...
            >
              ...
              <div
                className={`relative w-full overflow-hidden bg-stone-100 dark:bg-stone-800 ${trek.slideImages?.length > 0 ? "" : "aspect-[4/3]"}`}
              >
                ...
                <div
                  className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg font-label text-[10px] uppercase tracking-wider font-bold backdrop-blur-sm ${difficultyOverlayClass(trek.endurance_level)}`}
                >
              ...
              <div className="p-4 flex flex-col gap-2 flex-1">
                ...
                <div className="border-t border-stone-100 dark:border-stone-800 pt-2 mt-auto">
                  {trek.blog_link ? (
                    ...
                    <span className="text-stone-300 dark:text-stone-600 text-xs font-label">
                      No blog
                    </span>
                  )}
                </div>
